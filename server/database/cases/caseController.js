...
module.exports = {
  lookup: function (req, res, next) {
    ...
    if(userType){
      var Query = 'SELECT cases.type, users.name, users.email, users.phone_number, users.gender, patients.address FROM cases, users, patients WHERE cases.UserId = patients.id AND cases.UserId = users.id  order by cases.createdAt DESC';
